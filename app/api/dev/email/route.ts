...
// DEV ONLY. Preview / test-send the step-3 emails without spamming customers.
//   GET  ?jobId=X&kind=confirmation|same_day[&part=html|text|ics]   -> render, no send
//   POST { jobId, kind, to }                                        -> real send to `to`
function guard() {
...
export async function GET(req: Request) {
  ...
  const built =
    kind === "same_day"
      ? await buildSameDayBookingAlert(jobId)
      : await buildBookingConfirmationEmail(jobId);
...
export async function POST(req: Request) {
  ...
  const sent = await sendBookingConfirmationEmail(body.jobId, { overrideTo: body.to });
